# This will detect the target with the camera
...
import serial
import numpy as np
from yolov7.detect import YOLOv7
...
while True:
    ...
    # Our operations on the frame come here
    # Convert frame to RGB (Yolov7 expects RGB images)
    ...
    for detection in detections:
        x, y, w, h, class_id, score = detection
        cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 255, 0), 2)
        cv2.putText(frame, f'{yolo.classes[class_id]}: {score:.2f}', (x, y - 10),
    ...
    # If detections exist, send serial commands
    if detections:
        target = detections[0]  # Assuming we are interested in the first detected object
        target_center_x = target[0] + target[2] / 2
        target_center_y = target[1] + target[3] / 2

        err_x = 30 * (target_center_x - frame_w/2) / (frame_w/2)
        err_y = 30 * (target_center_y - frame_h/2) / (frame_h/2)
